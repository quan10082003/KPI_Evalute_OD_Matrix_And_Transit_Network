...
from domain.domain_service.kpi_service.kpi_service import Tranfer_rate_caculate, Cricuity_index_caculate

def test_calculate_transfer_rate_kpi():
    leg1 = AggregatedLeg("R1", {"S1"}, {"S2"})
    leg2 = AggregatedLeg("R2", {"S2"}, {"S3"})
    
    calculator = Tranfer_rate_caculate()
    
    # 0 transfer
    agg_iti_0 = AggregatedItinerary([leg1])
    res_0 = calculator.calculate_kpi(agg_iti_0)
    assert res_0["score"] == "0"
    
    # 1 transfer
    agg_iti_1 = AggregatedItinerary([leg1, leg2])
    res_1 = calculator.calculate_kpi(agg_iti_1)
...
def test_calculate_cricuity_index_kpi():
    ...
    calculator = Cricuity_index_caculate()
    
    iti_valid = Itinerary([Leg("R1", "S1", "S2")])
    res_valid = calculator.calculate_kpi(iti_valid, [route1])
    ...
    iti_empty = Itinerary([])
    res_empty = calculator.calculate_kpi(iti_empty, [route1])
